function DBFormDlg (ui) {
    ...
    this.ui = ui;
    thisForm.createEditButton(2, 
                                                "Insertar Gasto Abreviado", 
                                                "Inserta un gasto abreviado", 
                                                ":/generales/images/edit_add.png", 
                                                DBForm.CREATE, 
                                                "facturasprovgastos.dbrecord.ui", 
                                                "facturasprovgastos.dbrecord.qs");
...
DBFormDlg.prototype.beforeDelete = function(bean) {
    ...
    if ( efectos.length > 0 ) {
        ...
        if ( efectosOtrosDocumentos ) {
            AERPMessageBox.information("ATENCIÓN: Esta factura tiene efectos de pago asociados de otros documentos (albaranes). Borrarla eliminaría también esos efectos, desvirtuando la tesorería y la información de otros documentos(albaranes). Debe usted modificar la factura y eliminar los efectos que no pertenecen a esta factura para poder borrar");
            return false;
        }
        var ret = AERPMessageBox.question("ATENCIÓN: Esta factura tiene efectos de pago asociados, y puede tener movimientos de caja o banco asociados. Borrándolos, borrará también esos movimientos y los efectos de pago. Esto puede afectar a tesorería. ¿Está seguro de querer continuar borrando?", AERPMessageBox.Yes | AERPMessageBox.No);
...
DBFormDlg.prototype.exportarAsientos = function() {
    ...
    AERPMessageBox.information("Este script exportará en formato XBASE de Contaplus los asientos contables generados de las facturas por la empresa: " + empActual.nombre.value);

    var fecha1 = AERPScriptCommon.getDate("Introduzca la fecha de inicio desde la escoger las facturas recibidas a exportar.");
    if ( fecha1 == undefined ) {
        return;
    }
    var fecha2 = AERPScriptCommon.getDate("Introduzca la fecha de finalización desde la escoger las facturas recibidas a exportar.");
    if ( fecha2 == undefined ) {
        return;
    }
    ...
    var beans = AERPScriptCommon.beans("facturasprov", "idempresa = " + empActual.id.value + " and fecha >= " + AERPScriptCommon.sqlDate(fecha1) +
    ...
    for (var i = 0 ; i < beans.length ; i++ ) {
        ...
        if ( factura.co_asientos.brother != null ) {
            for ( var line = 0 ; line < factura.co_asientos.brother.co_partidas.items.length ; line++ ) {
                ...
                if ( subcuentaLinea.startsWith("400") || subcuentaLinea.startsWith("410") ) {
